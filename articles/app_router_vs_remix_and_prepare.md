...
## 不足の機能
まずは、App Routerを使う上で、補っておかなければならない機能について言及する。
App Router + Auth.jsで作成したが、Remix + remix-authでも同様のケアをすべきものなので、ライブラリ移行しても適用できるはずだ。
...
UIを書く上で、少なからず書き方を覚えていかなくてはならないが、ReactはJavaScriptを書くということの延長で書けることが素晴らしい。
...
他のactionsを用意する場合は、URL設計をして別のAPI Routes（Remix上の単語としてはResource Routes）を用意する必要がある。
...
反面、クライアントサイドでは利用できないので、その場合はAPI RoutesでEndpointを作り、`useEffect`で取得しなくてはならない。

筆者は今回無限スクロールの実装があったので、初期表示の際にデータ取得するだけではなく、スクロール時にデータ取得する必要があった。
こういった画面ではRSCではなくClient Componentが必要になる。

Remixでのデータ取得にはloader関数があり、特定の画面にloader関数が紐づいているようなので、わざわざAPI Routesを作らなくても無限スクロールの実装ができるようだ。
...
唐突にRemixへの言及が出てくる。なんとなく、感想パートの結語も予想がついている読者もいるかもしれない。
...
RemixならばAuth.jsを利用できるが、remix-authもある。
...
RSCは非常におもしろい機能で、利用してみてとても学びがあった。
だが、知れば知るほどNext.jsは様々な機能があり、お腹いっぱいというのが感想だ。
隣の芝生は青く見えるというが、機能的にはRemixのほうが絞られ、扱いやすそうだという印象を持っている。
今後は、Remixを使ってみたい。

ただ、もし仕事で採用するなら、もっと他の細かい機能要件を比較し、不足がないことを確認する必要がある。以下のRemix乗り換えの記事の内容までは、この記事では言及できていない。
