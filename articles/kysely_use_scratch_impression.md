...
そしてパフォーマンスチューニングする際にも、直接的にSQLを操作できるので、JSON Settingよりは楽かもしれない。
...
このカテゴリのSQLは、JSON Settingで実装しやすく、Query Builderでは冗長に感じるはずだ。
...
Prismaは1,2はJSON Settingで対応し、3,4はRaw SQLで対応する。kyselyは4のみRaw SQLだろう。
それぞれのSQLの表現方法によって、1-4のカテゴリに強いか弱いかを導いている。動的に組み立てないならPrismaのほうが圧倒的だ。
