...
- fetchの件はパッチ自体ではなく、パッチの当て方
- spaと静的exportは対照的な機能
- remixのほうができることが少ないというか、絞っていて、絞った選択肢の先で工夫する
- 静的exportも動的パスに限界があるので、あきらめてspaモードに割りきるとか
- remixは組み込みサーバでも動くので、徹底してプログラマがコントローラブルなのも好印象
