...
だが、どちらも筆者の手に馴染む道具とはならなそうだったので、探したところgorpとgoquを組み合わせるに至った。  
...
後段で実装の詳細がでてくるが、要求事項は選んだ理由や設計ポリシーが関わる重要な部分である。  

そもそも大前提として、DBアクセスの詳細は隠蔽されていたほうがいい。  
より具体的には、DB操作というのは、特定の関数をinterfaceとして、引数を与えることで指示が出せる状態が望ましい。  

これによって、テストを行う際にDBアクセス部分は切り離して独立させ、DBを意識しないテストができる。  
...
上記のsqlを実現するだけでも、GORMなら以下のようにチェーンが伸びる。GORMは関数チェーンを用いなくても書くことはできるが、例として提示する。  
```go
...
キーで検索する場合は、以下のようなinterfaceの実装となるだろう。  
```go
...
これであれば、`PostRepository`などから直接この用途で利用され、今後拡張されることもない。また、非常に多くの場面で利用される。  
...
### DBアクセスライブラリに求めるもののまとめ

ここまで簡単なsqlの定義を行ってきたが、この簡単なsqlは、開発プロジェクトの中で頻繁に出てくる。  
簡単なsqlを`PostDataAccess`のようなモジュールを定義せずとも簡単に実現できて、複雑なsqlは`PostDataAccess`でラップして実現する。というのが、筆者の基本方針だ。  

簡単なsqlは、非常に簡単なので単体テストで品質保証を行うモチベーションが低いので、メタコードで実現されていても構わない。  
むしろ開発プロジェクトで頻繁にでてくるので、メタコードで再利用性高く実装されていてほしい。  
それ以外の複雑なsqlは、多かれ少なかれロジックがあるので、DBアクセスを伴う単体テストで部品の品質保証をしたい。  

## 実装
ここからは、実際にgorp+goquの良さや、使い方について書いていく。  
...
構造体のTagに、取得のカラム名を定義しておけば、その構造体を操作するようにテーブルのCRUDができる。  
...
generateするのは、開発に1ステップ挟むことなので、体験が悪いイメージがある（でもイメージだけだ 実際は使ってみて評価すべきだろう）。  
...
実のところ、主キーだけではなく、ユニークキーで取ってくる場合もあるので、schema情報からキーを決定できない。  
...
複合主キーの場合、それぞれのカラムがランダムということは少ない。特定のカラムの中で、別のカラムがユニークになっているケースが多い。  
...
select *
 ...
 where author_id in (?)
   and post_id in (?, ?, ?)
     ;
```
上記であれば、and条件とin句のみで実現できる。author_idの中でpost_idが一意になっているので、author_idは一つだけ指定することで、問題なく利用できる。  
...
また、複合主キーでor句を条件としないといけないsqlは1度だけでてきた。1度だけならば、単純にそれ専用のsqlをかいて、XxxDataAccessでラップして、しっかり単体テストを書いて検証するほうがいいだろう。  
...
キー発行について、どれが優れているかはここでは言及しないが、uuidやintを使ったり、またデータベース上での発行、アプリケーション上での発行といくつかパターンがある。  
...
特定のテーブルの単一の主キーとして、アプリケーションで発行するintのキーは衝突の可能性としてもっと考慮が必要だが、グループコード内であれば、ユーザがグループ内での活動をコントロールできるので、衝突時のハンドリングを外部化してもいいだろう。  

`GetMax`はテーブルのint主キーの最大値を取ってくるために定義したメソッドだ。キー設計はプロジェクトによって違うので、プロジェクトによっては不要なものだろう。  
...
定義した関数の引数も多いので、使い方もぶれて行きそうだ。  
やはり筆者としては不要かなという印象が強い。  
...
```go
type PostDataAccess interface {
	Executor // 前段でgorp.SqlExecutorを拡張したオブジェクト
...
func (pr postDataAccess) GetPostByComenntUser(userId int, content string) ([]Post, error) {
...
`postDataAccess`単独でsqlをテストできるので、sql部分のテストは独立して評価できるようになる。  
筆者はdockertestというツールを利用して、goのコードからpostgresqlをdocker上で立ち上げてテストする形を取っている。  
https://github.com/ory/dockertest

DBレコードに対応する構造体を定義しているので、それらの構造体を利用してレコードをテストファイル上に定義して、実行している。  
...
[Bob](https://github.com/stephenafamo/bob)はコードgenerateがなくても、goquのような表現力がありそうだったし、筆者とは微妙に違うが、sqlを扱うモジュールのレイヤを意識した設計になっているようだ。  
