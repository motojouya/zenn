...
逆に、複合主キーでor句を条件としないといけないsqlは1度だけでてきた。1度だけならば、単純にそれ専用のsqlをかいて、XxxDataAccessでラップして、しっかり単体テストを書いて検証するほうがいいだろう。  
...
この`GetMax`が必要と感じるかは、利用者次第である。`GetMax`はテーブルのint主キーの最大値を取ってくるために定義したメソッドだ。  
...
postDataAccess単独でsqlをテストできるので、sql部分のテストは独立して評価できるようになる。  
