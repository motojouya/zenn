...
## 利用
エラーハンドリングについて、様々なStyleを挙げてきた。筆者はGolang Styleで書いているが、実際にはどうしてもTry Catch Styleが発生する場面が存在する。
...
こういった利用者が回復不可能なエラーは、Golang Styleで丁寧にreturnするよりも、Try Catch Styleで`Error class`をthrowしてcatchし、stack traceを出力するほうが良いだろう。

つまり、エラーの種類や、発生場所によってはTry Catch Styleで実装する必要があるということだ。
筆者は、ライブラリは必ずwrapするコードを用意して、コアロジックがライブラリに依存しない形でコードを書くようにしている。
この際、このwrapする関数上で、ライブラリからthrowされてきたエラーを、自分で実装したエラーの形式に変換して利用する。
すべてのライブラリでこの作業をするのは大変だが、エラーハンドリング以外の場合でも、様々ないい効果があるので、筆者としてはおすすめする。

上記をした上で、必要な場面ではTry Catch Styleで、その他のコードではGolang Styleで書いている。
NeverThrowが気に入っている読者も、try catchの要素を含まなくてはならない場面もあるはずで、それがどこなのか、どういうときに使い分けるのかも整理して利用してほしい。
