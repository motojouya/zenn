...
個別の集約の中でも、依存しているものは示しているが、全体感は知りたいところだ。  
...
ここまで説明してきたクラス図は、実装前の詳細設計のドキュメントになる。この資料で表現される集約、構造体、また振る舞いがあれば、機能の実装をイメージしやすい。  
まさに詳細設計書を用いて、機能をどう実装するか検討する資料である。  
クラス図だけでコードがかけるわけではないが、コードベースの構造を定義して検証するのにクラス図は大いに役に立つと考えている。

## Layerについて
Package By Aggregateとしたが、その下にLayerも切ると述べた。それについても言及すべきだろう。  
Layerの切り方は色々とあるが、とりあえず、簡単に以下のように考えたい。  
...
ただ筆者は、特定のControllerの機能は、特定のAggregateを話題の中心として機能を提供する傾向があると考えている。  
...
構造体の視点では、依存関係は一定方向だが、集約同士の視点では`在庫`と`棚卸し`が循環している。  
