...
例を出そう。冒頭で会社のリポジトリでやっていると言ったので、株式会社オープンロジのデータモデルを簡略化したものを例示したい。  
オープンロジは、物流システムを提供している。ものすごく簡単に言うと倉庫に預けて、出すまでが基本的な守備範囲である。  
...
### 荷主
まずは荷主から。荷主は、オープンロジにおいてはお客様と同義なので、アカウントと読み替えてもいい。  
...
それよりも、その集約の振る舞いを定義し、機能、つまりドメイン駆動設計などで言うところのアプリケーション層の実装イメージをつけやすくすることが、ここの意図だ。  
...
倉庫はオープンロジが管理しているものなので、勝手に作ることができない。アプリケーション上で公開すべきふるまいは`Update倉庫住所`のみだ。  
...
ECにおいて在庫を注文に割り当てることを`引当`というのは割と一般的と考えているが、この引当はシステム的な負荷が高く、オープンロジでも難しい処理だ。  
...
### クラス図以外のドキュメント
ここまでクラス図の説明をしてきたが、他にもwebシステムならroutingの定義、画面遷移の定義、DBスキーマ設計なども必要になる。  
...
- Controller  
  ユースケースとしてもいいかもしれない  
  重要なのは、ドメイン層を利用するアプリケーション層ということだ  
- Model  
  データモデルの項目を定義し、ロジックなども持つ  
  いわゆるドメインモデルだろう。これはドメイン層だ。  
- Repository  
  DBアクセスを隠蔽するのにわかりやすい概念だろう  
  これはドメイン層のものだ  

ドメイン層はドメインを表現するもの、アプリケーション層は、ドメインを利用して機能を実現するものと定義しよう。  
...
筆者もControllerはアプリケーション層と定義した。その上で、ドメイン層である集約の中のPackageとしている。  
...
こういった考えに基づけば、アプリケーション層のコードを、特定の集約Packageに入れ込むのも悪くないと考える。  
また、仮にアプリケーション層とドメイン層で分けようとすると、ディレクトリはもう一段深くなるだろう。それもまた面倒だ。  
...
## まとめ
今回はどんなふうにUMLを書き、それを受けてPackage構成をどうするかという話題で記事を書いた。またそれらを検討する際のキーワードは`集約`である。  
