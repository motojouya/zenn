...
TODO インラインの引用を多様しているが、これはコード中の単語の引用とかタイトルの引用に限りたい。アルファベットなので目立つし、それで十分なので
...
このようなときは`name`項目は`Name`型とすることはできないだろう。
...
本記事ではEntityをどこにも依存せず、DBのことを知らないオブジェクトと述べた。したがって、Repositoryは引数、返り値にEntityを持ち、DBアクセスなどを抽象化する役割と見ていいだろう。  
...
もちろん、データ構造に手を入れたりすることで、上記を改善することもできるが、筆者個人の経験としてはある程度システムが複雑になってきたときに、詰め替える仕組みがあったほうが整理しやすい。  
...
例えば、データモデルが変更になるのであれば、それはおそらく`Entity`にも波及するだろう。それは単にデータモデルが変化するという話だ。  
